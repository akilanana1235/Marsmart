  ...
  _allProducts.forEach((product) => {
    if (product.id == id) {
      outputProductCard += `
            ...
            <img src=${product.img1} style="width:15em;margin-left:12%;" alt="">
            <div>
              <h5 class="product-card-name">${product.name}</h5>
              <h6>
                <span>${product.category}</span>
                <span style="padding-left:6.5em;margin-left:32%;"><i class='fa fa-star checked' aria-hidden='true'></i>${calculateProductRating(parseInt(product.id))}</span>
              </h6>
            </div>
